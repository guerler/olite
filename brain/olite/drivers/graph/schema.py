...
class BaseNode(BaseModel):
    ...
    next: str | None = None
    # Outcome-routing: maps "ok"/"error"/"warning" to a next node. The runner reads
...
class AgentDefinition(BaseModel):
    ...
    def validate_json_planner_targets(self) -> "AgentDefinition":
        """Constrain what a JSON planner may feed."""
        allowed_targets = {"materializer", "compute", "planner", "executor", "reasoning", "terminal"}
